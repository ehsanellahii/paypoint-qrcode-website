            ...
            return (
              <button
                ...
                <div className='relative w-10 h-10 rounded-full overflow-hidden'>
                  {categoryImage && <Image src={categoryImage} alt={category.name} fill className='object-contain' sizes='40px' />}
                </div>
                <span className={`text-xs font-medium text-center ${isActive ? 'text-black' : 'text-gray-700'}`}>{category.name}</span>
              </button>
            );
          })}
        </div>
